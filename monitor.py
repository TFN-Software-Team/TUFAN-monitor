...
def serial_worker(data_queue, stop_event):
    ...
    try:
        ser = serial.Serial(config.SERIAL_PORT, config.SERIAL_BAUD, timeout=2)
    except serial.SerialException:
        print(f"Port {config.SERIAL_PORT} açılamadı. config.py'de SERIAL_PORT'u kontrol edin.")
        log_file.flush()
        log_file.close()
        return
...
class MonitorApp:
    ...
    def set_link_status(self, connected):
        if connected:
